...
| 검사 | 복잡도 | 비고 |
|------|--------|------|
| StepMap 선분 검사 (3D DDA) | O(Δx + Δy + N) | 선분이 통과하는 격자 수. Δx·Δy는 공간 해상도·거리에 비례, N은 시간 스텝 수. 장애물 수와 무관 |
| 동적 장애물 skew-line 검사 | O(M × N) | M = 동적 장애물 수, N = 시간 스텝 수. 스텝당 O(1) 내적·노름 연산 |

**장단점 요약:**
- StepMap DDA는 장애물 수 M에 독립적이므로, M이 클수록 상대적으로 유리하다.  
  단, 격자 해상도가 낮거나 공간 범위가 넓으면 Δx + Δy 항이 커진다.
- 동적 장애물 검사는 StepMap에 포함되지 않은 원형 장애물의 정밀한 반경 판별을 제공하지만,  
  M × N에 정비례하여 증가한다.
- StepMap 검사가 먼저 실행되며 **비가시** 판정 시 즉시 반환하므로,  
  정적·동적 장애물이 모두 StepMap에 반영된 경우 동적 장애물 루프를 건너뛸 수 있다.
